<template>
    <card class="px-6 py-4">
    	<div class="flex mb-4">
    		<h3 class="mr-3 text-base text-80 font-bold">{{ card.name || 'Countdown' }}</h3>
    	</div>
    	<p class="flex items-center text-4xl mb-4">{{ timeLeft }}</p> 
    	<div class="flex items-center">
    		<p class="text-80 font-bold" v-if="card.label"><span>{{ card.label }}</span></p>
    	</div>
    </card>
</template>
